...
path = r'D:\DATA\NC_2023_HE_04.xlsx'
project_number = 'NC_2023_HE_04'
...
for index, row in df.iterrows():
    ...
    if pd.isna(row['开始时间']) is False:
        ...
        t = row['开始时间2'] if not pd.isna(row['开始时间2']) else '00:00:00'
    ...
    values = (project_number, experiment_name, electrodeID, start_time, duration, record_way, file_type, path, experimenter,
              sample_rate, record_system, paradigm, signal_type, channel_count, invasive_date, region, depth)
...
sql = ('INSERT INTO session (project_number, experiment_name, electrodeID, start_time, duration, record_way, file_type, path, experimenter,'
       'sample_rate, record_system, paradigm, signal_type, channel_count, invasive_date, region, depth)  VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)')
